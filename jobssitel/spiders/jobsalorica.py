...
# for this site need to change the ROBOTSTXT_OBEY parameter to False in the setting.py

class GenericSpider(scrapy.Spider):
    name = 'jobsalorica'
    start_urls = ['https://jobs.alorica.com']
    start_page_url = 'https://jobs.alorica.com/ListJobs/All/Search/AL-Primary-Location/United%20States/AL-Job-Function/Call-Center/'
    job_company = 'Alorica'
    scraped_items = 0
    total_items = 0
    items_urls = []
    ...
    # for this site in each request need to add the 'User-Agent' parameter to the headers array to make it look like a real browser
    # otherwise the site returns error '503 Service Unavailable'
    headers = {
    ...
    def start_requests(self):
        ...
        yield Request(self.start_page_url, dont_filter=True, headers=self.headers)
        # yield Request(
        #     'https://jobs.alorica.com/ShowJob/Id/1267930/Technical-Support-Customer-Service-Agents-Apply-Today!!/',
        #     self.parse_job_page, dont_filter=True, headers=self.headers)
    ...
    def parse(self, response):
        ...
        table_rows = response.xpath('//table[@class="JobListTable"]/tr')
        for table_row in table_rows:
            ...
            job_url = table_row.xpath('./td[@class="coloriginaljobtitle"]/a/@href').get()
            if not job_url:
                continue
            ...
            if job_url not in self.items_urls:
                ...
                yield Request(job_url, self.parse_job_page, dont_filter=True, headers=self.headers)
        ...
        if next_page:
            ...
            if next_page not in self.page_urls:
                ...
                yield Request(next_page, dont_filter=True, headers=self.headers)
    ...
    def parse_job_page(self, response):
        ...
        job_req_id = response.xpath('//span[@itemtype="http://schema.org/JobPosting"]/div[1]/text()').get()
        if job_req_id:
            job_req_id = job_req_id.replace('Requisition ID: ', '').strip()

        # job_category = response.xpath('//span[@itemprop="occupationalCategory"]/text()').get()
        # if job_category:
        #     job_category = job_category.replace('Job Category: ', '').strip()

        job_location = response.xpath('//span[@itemprop="jobLocation"]/span/span/text()').get()
        if job_location:
            job_location = job_location.replace('Location: ', '').strip()

        job_posted_date = response.xpath('//span[@id="date"]/text()').get()
        if job_posted_date:
        ...
        if description and len(description) > 0:
            ...
            job_description_clean = re.sub(' +', ' ', "\n".join(response.xpath('//div[@itemprop="description"]//text()').extract())).strip()
